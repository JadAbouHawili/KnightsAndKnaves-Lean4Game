...
-- *
macro "knight_to_knave" t2:term "at"  t1:Lean.Parser.Tactic.locationWildcard : tactic =>
do`(tactic| simp [inleft_notinrightIff (either $t2) dis] at $t1)
-- goal
macro "knight_to_knave" t2:term : tactic =>
do`(tactic| simp [inleft_notinrightIff (either $t2) dis])
-- hypothesis
macro "knight_to_knave" t2:term "at" t1:Lean.Parser.Tactic.locationHyp : tactic =>
do`(tactic| 
simp [inleft_notinrightIff (either $t2) dis] at $t1)

-- *
macro "knave_to_knight" t2:term "at"  t1:Lean.Parser.Tactic.locationWildcard : tactic =>
do`(tactic| simp [inright_notinleftIff (either $t2) dis] at $t1)
-- goal
macro "knave_to_knight" t2:term : tactic =>
do`(tactic| simp [inright_notinleftIff (either $t2) dis])
-- hypothesis
macro "knave_to_knight" t2:term "at" t1:Lean.Parser.Tactic.locationHyp : tactic =>
do`(tactic| 
