...
(proof by cases)
  ...
  - Use `constructor` tactic to split the goal in two, the first being `A` and the second being `B ∧ C`
  - Use `⟨⟩` notation
  - Use `And.intro`
  - Use `have` to first construct a proof of `B ∧ C` then use `And.intro`
  - etc...
