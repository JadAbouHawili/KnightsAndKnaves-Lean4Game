...
(proof by cases)
  ...
  Use `constructor` tactic to split the goal in two, the first being `A` and the second being `B ∧ C` or use `⟨⟩` notation or use `And.intro` or use `have` to first construct a proof of `B ∧ C` then use `And.intro` etc...
